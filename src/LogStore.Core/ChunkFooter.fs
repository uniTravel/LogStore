...
module ChunkFooter =
    ...
    let buffer = Array.zeroCreate <| int (size - checksumSize)
    ...
    let fromStream (OriginStream stream) =
        ...
        let hash = reader.ReadBytes (int checksumSize)
    ...
    let accHash (md5: MD5) (OriginStream stream) =
        ...
        md5.TransformFinalBlock (buffer, 0, buffer.Length) |> ignore
    ...
    let complete dataSize (md5: MD5) =
        let footer = { _isCompleted = true; _dataSize = dataSize; _md5Hash = Array.empty }
        md5.TransformFinalBlock(asByteArray footer, 0, buffer.Length) |> ignore
